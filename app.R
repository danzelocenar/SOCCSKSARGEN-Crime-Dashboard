...
# Optimized for Posit Connect Cloud deployment
# ============================================================
...
# ============================================================
# DATA LOADING
# - Spatial objects are pre-built RDS (no heavy computation at startup)
# - crime_raw is dropped immediately after cleaning to free RAM
# ============================================================
...
cp_results <- tryCatch(
  read_excel("Objective3_CPA_Results.xlsx", sheet = "CP_Monthly_Comparison"),
  error = function(e) NULL
)
prov_cp <- tryCatch(
  read_excel("Objective3_CPA_Results.xlsx", sheet = "Province_CP_Monthly"),
  error = function(e) NULL
)
gc()

# ── Spatial objects — pre-built by posit_deploy.R, no computation here ──
...
# Identify best change-point (new monthly sheet flags it with "Best CP").
# best_cp_year = integer year; best_cp_label = display string e.g. "Jul 2018".
best_cp_row <- (
  if (!is.null(cp_results) && "Best" %in% names(cp_results))
    cp_results %>% filter(Best == "Best CP") %>% slice(1)
  else if (!is.null(cp_results))
    cp_results %>% filter(WAIC == min(WAIC, na.rm = TRUE)) %>% slice(1)
  else NULL
)

best_cp_year  <- if (!is.null(best_cp_row) && nrow(best_cp_row) > 0)
  as.integer(best_cp_row$CP_Year[[1]]) else NULL
best_cp_label <- if (!is.null(best_cp_row) && nrow(best_cp_row) > 0)
  as.character(best_cp_row$CP_Label[[1]]) else NULL
best_cp_month <- if (!is.null(best_cp_row) && nrow(best_cp_row) > 0)
  as.integer(best_cp_row$CP_Month[[1]]) else NULL
...
# ── Save-map builder — lazy, only called inside downloadHandler ──
build_save_sf <- function() {
  totals <- crime_clean %>%
    group_by(Province) %>% summarise(Total = n(), .groups = "drop")
  sf_out <- region_sf %>% left_join(totals, by = "Province")
  if (!is.null(bhm_rr))
    sf_out <- sf_out %>%
    left_join(bhm_rr %>%
...
build_save_popup <- function(sf_obj) {
  ...
  mapply(function(prov, total, rr, rr_lo, rr_hi, risk, cp_yr, rr_chg, sig) {
    rr_html <- if (has_rr && !is.na(rr)) paste0(
      "<br><br><b style='color:#94a3b8;font-size:11px;'>RELATIVE RISK (BHM)</b><br>",
    ...
    cp_html <- if (has_cp && !is.na(cp_yr)) paste0(
      "<br><br><b style='color:#94a3b8;font-size:11px;'>CHANGE-POINT YEAR</b><br>",
      "<span style='font-size:20px;font-weight:700;color:#10b981;'>", cp_yr, "</span>",
      "<br><span style='color:#64748b;font-size:11px;'>Crime change: ",
      round((rr_chg - 1) * 100, 1), "% | Significant: ", sig, "</span>"
    ) else ""
    paste0(
      "<div style='font-family:Inter,sans-serif;background:#1e293b;color:#e2e8f0;",
      "padding:14px;border-radius:8px;min-width:240px;'>",
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "SOCCSKSARGEN Crime Analysis", titleWidth = 300),
  ...
  dashboardSidebar(
    ...
    tags$div(
      ...
      sliderInput("selected_year", NULL,
      ...
      pickerInput("category", NULL, choices = all_categories,
                  selected = "All Categories",
                  options = pickerOptions(style = "btn-sm"), width = "100%"),
      
      tags$span("Province", class = "filter-label"),
      pickerInput("province", NULL, choices = all_provinces,
                  selected = "All Provinces",
                  options = pickerOptions(style = "btn-sm"), width = "100%"),
      
      tags$span("Month", class = "filter-label"),
      pickerInput("month_filter", NULL, choices = all_months,
                  selected = "All Months",
                  options = pickerOptions(style = "btn-sm"), width = "100%"),
      
      br(),
      actionBttn("reset", "Reset All Filters",
                 style = "bordered", color = "primary",
                 size = "sm", block = TRUE, icon = icon("undo")),
      br(),
      downloadButton("save_html", label = "Save as HTML File",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "overview",
              ...
              fluidRow(
                column(width = 4,
                       tags$div(class = "story-card", tags$h4("About This Study"),
                                tags$p("This dashboard presents crime patterns across the 5 provinces
                      and city of SOCCSKSARGEN (Region XII), Philippines, from 2015
                      to 2025. Navigate through the tabs to explore crime hotspots,
                      trends, and major structural changes.")),
                       tags$div(class = "story-card", tags$h4("Major Events in the Region"),
                                tags$p(
                       ...
                       tags$div(class = "story-card", tags$h4("How to Use This Dashboard"),
                                tags$p("Use the sidebar filters to explore specific years, provinces,
                      and crime categories. The Crime Map tab has a Play button to
                      animate crime patterns year by year. Click any province on
                      any map for details."))
                ),
                column(width = 5,
                       box(title = "Crime Hotspot Map (2015-2025)",
                           status = "primary", solidHeader = TRUE, width = NULL, height = 420,
                           leafletOutput("overview_map", height = "360px"))),
                column(width = 3,
                       box(title = "Crime Share by Province", status = "info",
                           solidHeader = TRUE, width = NULL, height = 200,
                           plotlyOutput("ov_prov_pie", height = "150px")),
                       box(title = "Crime Share by Category", status = "warning",
                           solidHeader = TRUE, width = NULL, height = 200,
      ...
      tabItem(tabName = "map",
              ...
              fluidRow(
                column(width = 9,
                       box(title = uiOutput("map_header"), status = "primary",
                           solidHeader = TRUE, width = NULL, height = 580,
                           radioGroupButtons("map_var", label = NULL,
                                             choices  = c("Crime Count" = "count",
                                                          "Relative Risk" = "rr",
                                                          "Change-Point Year" = "cp"),
                                             selected = "count", size = "sm", status = "primary", justified = TRUE),
                           br(),
                           leafletOutput("crime_map", height = "470px"))),
                column(width = 3,
                       box(title = "Province Ranking", status = "info",
                           solidHeader = TRUE, width = NULL, height = 290,
                           plotlyOutput("map_prov_bar", height = "240px")),
                       box(title = "Category Split", status = "warning",
                           solidHeader = TRUE, width = NULL, height = 270,
      ...
      tabItem(tabName = "category",
              fluidRow(
                ...
                box(title = "Crime Category Share", status = "info",
      ...
      tabItem(tabName = "risk",
              fluidRow(column(width = 12,
                              tags$div(class = "story-card", tags$h4("What is Relative Risk?"),
              ...
              fluidRow(
                ...
                box(title = "Province Risk Ranking", status = "warning",
                    solidHeader = TRUE, width = 5,
                    tags$p(style = "color:#94a3b8;font-size:12px;margin-bottom:8px;",
                           "Red = above average risk. Green = below average risk. Error bars show uncertainty range."),
      ...
      tabItem(tabName = "data",
              fluidRow(
                box(title = "Browse the Crime Data", status = "primary",
                    solidHeader = TRUE, width = 12,
                    tags$p(style = "color:#94a3b8;font-size:12px;margin-bottom:8px;",
                           "Use the search boxes below each column to filter. Use the buttons above to download the data."),
                    DTOutput("data_tbl")))
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$reset, {
    updateSliderInput(session, "year_range",    value = c(min(all_years), max(all_years)))
    updateSliderInput(session, "selected_year", value = max(all_years))
  ...
  output$save_html <- downloadHandler(
    ...
    content  = function(file) {
      ...
      map_save <- leaflet(save_sf_data) %>%
        ...
        addPolygons(fillColor = ~pal_save(Total), fillOpacity = 0.80,
                    color = "#0f172a", weight = 2, group = "Crime Count",
                    popup = popups,
                    highlightOptions = highlightOptions(weight = 3, color = "#3b82f6",
                                                        fillOpacity = 0.95, bringToFront = TRUE),
                    label = ~paste0(Province, ": ",
                                    formatC(Total, format = "d", big.mark = ","), " crimes")) %>%
        addLegend(pal = pal_save, values = ~Total,
                  title = "Total Crimes (2015-2025)", position = "bottomright", opacity = 0.85) %>%
        addLabelOnlyMarkers(data = centroids_df, lng = ~cx, lat = ~cy, label = ~Province,
                            labelOptions = labelOptions(noHide = TRUE, textOnly = TRUE,
                                                        style = list("color" = "white", "font-weight" = "bold",
                                                                     "font-size" = "12px",
                                                                     "text-shadow" = "0 0 6px #0f172a,0 0 6px #0f172a"))) %>%
        addLayersControl(baseGroups = c("Dark Map", "Light Map", "Satellite"),
                         overlayGroups = "Crime Count",
                         options = layersControlOptions(collapsed = FALSE),
                         position = "topright") %>%
        addScaleBar(position = "bottomleft") %>%
        addMiniMap(tiles = providers$CartoDB.Positron, toggleDisplay = TRUE,
                   minimized = FALSE, width = 150, height = 150) %>%
        addControl(html = paste0(
          "<div style='background:#1e293b;color:#e2e8f0;padding:12px 16px;",
          "border-radius:10px;border:1px solid #3b82f6;font-family:Inter,sans-serif;",
          "box-shadow:0 4px 20px rgba(0,0,0,0.4);max-width:280px;'>",
          "<b style='font-size:14px;color:#3b82f6;'>SOCCSKSARGEN Crime Analysis</b><br>",
          "<span style='font-size:11px;color:#94a3b8;'>Bayesian Spatio-Temporal Modeling",
          "<br>2015-2025 | Region XII, Philippines</span>",
          if (!is.null(best_cp_label)) paste0(
            "<br><span style='font-size:11px;color:#10b981;font-weight:600;'>",
            "Change-Point Identified: ", best_cp_label, "</span>") else "",
          "<br><span style='font-size:10px;color:#64748b;'>Click any province for details</span></div>"
        ), position = "topleft") %>%
        setView(lng = 124.7, lat = 6.4, zoom = 8)
      saveWidget(map_save, file = file, selfcontained = TRUE,
  ...
  fdata <- reactive({
    ...
    if (input$category != "All Categories") d <- d %>% filter(Category == input$category)
    if (input$province != "All Provinces")  d <- d %>% filter(Province == input$province)
  ...
  yr_data <- reactive({
    ...
    if (input$category != "All Categories") d <- d %>% filter(Category == input$category)
    d %>% group_by(Province) %>% summarise(Count = n(), .groups = "drop")
  })
  ...
  output$ov_cp <- renderValueBox(
    valueBox(ifelse(is.null(best_cp_year), "N/A", best_cp_year),
             "Year Crime Pattern Changed", icon = icon("bolt"), color = "green"))
  output$map_count <- renderValueBox(
    valueBox(formatC(nrow(fdata()), format = "d", big.mark = ","),
             "Incidents in Selection", icon = icon("exclamation-circle"), color = "red"))
  output$map_year <- renderValueBox(
    valueBox(input$selected_year, "Map Year", icon = icon("calendar"), color = "blue"))
  ...
  output$map_change <- renderValueBox(
    valueBox(ifelse(is.null(best_cp_year), "N/A", paste("CP:", best_cp_year)),
             "Change-Point Year", icon = icon("bolt"), color = "green"))
  
  output$map_header <- renderUI({
    v <- switch(input$map_var, "count" = "Crime Count",
                "rr" = "Relative Risk (BHM)", "cp" = "Change-Point Year")
    paste(v, "-", input$selected_year, "|", input$category)
  })
  
  # ── Overview map ──────────────────────────────────────────
  output$overview_map <- renderLeaflet({
    totals <- crime_clean %>% group_by(Province) %>% summarise(Total = n(), .groups = "drop")
    sf_ov  <- region_sf %>% left_join(totals, by = "Province")
    pal    <- colorNumeric("YlOrRd", domain = sf_ov$Total)
    leaflet(sf_ov) %>%
      addProviderTiles(providers$CartoDB.DarkMatterNoLabels) %>%
      addPolygons(fillColor = ~pal(Total), fillOpacity = 0.8,
                  color = "#1e3a5f", weight = 2,
                  popup = ~paste0(
                    "<div style='font-family:Inter;background:#1e293b;color:#e2e8f0;",
                    ...
                    "</span><span style='color:#94a3b8;font-size:12px;'> total crimes</span></div>"),
                  highlightOptions = highlightOptions(weight = 3, color = "#3b82f6",
                                                      fillOpacity = 0.95, bringToFront = TRUE)) %>%
      addLabelOnlyMarkers(data = centroids_df, lng = ~cx, lat = ~cy, label = ~Province,
                          labelOptions = labelOptions(noHide = TRUE, textOnly = TRUE,
  ...
  output$ov_prov_pie <- renderPlotly({
    ...
    plot_ly(d, labels = ~Province, values = ~n, type = "pie", hole = 0.5,
      ...
      dark_plotly() %>% layout(margin = list(t = 5, b = 5, l = 5, r = 5))
  })
  
  output$ov_cat_bar <- renderPlotly({
    d <- crime_clean %>% count(Category, sort = TRUE) %>%
      mutate(Category = str_wrap(Category, 20))
    plot_ly(d, x = ~n, y = ~reorder(Category, n), type = "bar", orientation = "h",
            marker = list(color = "#3b82f6", line = list(color = "#1e3a5f", width = 1)),
  ...
  observe({
    sf_yr <- region_sf %>% left_join(yr_data(), by = "Province") %>%
      mutate(Count = replace_na(Count, 0))
    var <- input$map_var
    
    if (var == "rr" && !is.null(bhm_rr)) {
      sf_yr   <- sf_yr %>% left_join(bhm_rr %>%
                                       dplyr::select(Province, RR, RR_lower, RR_upper, Risk), by = "Province")
      vals    <- sf_yr$RR
      ...
      ), sf_yr$Province, sf_yr$RR, sf_yr$RR_lower, sf_yr$RR_upper, sf_yr$Risk, SIMPLIFY = TRUE)
      
    } else if (var == "cp" && !is.null(prov_cp)) {
      sf_yr   <- sf_yr %>% left_join(prov_cp %>%
                                       dplyr::select(Province, Best_CP_Year, RR_Change, Significant), by = "Province")
      ...
      ), sf_yr$Province, sf_yr$Best_CP_Year, sf_yr$RR_Change, sf_yr$Significant, SIMPLIFY = TRUE)
      
    } else {
      vals    <- sf_yr$Count
      pal     <- colorNumeric(c("#0f172a", "#1d4ed8", "#f97316", "#dc2626"),
      ...
      popups  <- mapply(function(prov, cnt) paste0(
        ...
        "<span style='color:#94a3b8;font-size:11px;'>crimes in ", input$selected_year,
        "</span></div>"
      ), sf_yr$Province, sf_yr$Count, SIMPLIFY = TRUE)
    }
    
    leafletProxy("crime_map") %>% clearShapes() %>% clearControls() %>%
      addPolygons(data = region_muni, fillColor = "transparent",
                  color = "#1e3a5f", weight = 0.8, group = "Municipalities") %>%
      addPolygons(data = sf_yr, fillColor = ~pal(vals), fillOpacity = 0.80,
                  color = "#0f172a", weight = 2, popup = popups,
                  popupOptions = popupOptions(maxWidth = 280),
                  highlightOptions = highlightOptions(weight = 3, color = "#3b82f6",
                                                      fillOpacity = 0.95, bringToFront = TRUE),
                  label = ~paste0(Province, ": ", formatC(Count, format = "d", big.mark = ","))) %>%
      addLabelOnlyMarkers(data = centroids_df, lng = ~cx, lat = ~cy, label = ~Province,
                          labelOptions = labelOptions(noHide = TRUE, textOnly = TRUE,
                                                      style = list("color" = "white", "font-weight" = "bold",
                                                                   "font-size" = "11px",
                                                                   "text-shadow" = "0 0 4px #0f172a,0 0 4px #0f172a"))) %>%
      addLegend(pal = pal, values = vals, title = leg_ttl,
  ...
  output$trend_regional <- renderPlotly({
    ...
    if (!is.null(best_cp_year) &&
        best_cp_year >= min(d$Year) && best_cp_year <= max(d$Year)) {
      p <- p %>%
        ...
        add_annotations(x = best_cp_year, y = max(d$Count, na.rm = TRUE),
                        ...
                        showarrow = FALSE,
                        font = list(color = "#10b981", size = 11), xanchor = "left")
    }
    p %>% dark_plotly() %>%
      layout(xaxis = list(title = "", tickmode = "linear", dtick = 1),
             yaxis = list(title = "Total Crimes"), hovermode = "x unified", showlegend = TRUE)
  })
  ...
  output$trend_monthly <- renderPlotly({
    d <- fdata() %>% count(Month, Year, name = "Count") %>% mutate(Mo = month.abb[Month])
    plot_ly(d, x = ~factor(Mo, levels = month.abb), y = ~factor(Year), z = ~Count,
            type = "heatmap",
            colorscale = list(c(0, "#0f172a"), c(0.5, "#1d4ed8"), c(1, "#dc2626")),
            hovertemplate = "<b>%{y} - %{x}</b><br>%{z:,} crimes<extra></extra>",
            showscale = TRUE) %>%
      dark_plotly() %>%
      layout(xaxis = list(title = "Month"), yaxis = list(title = ""),
  ...
  output$cat_total_bar <- renderPlotly({
    ...
    plot_ly(d, x = ~n, y = ~reorder(Category, n), type = "bar", orientation = "h",
            ...
            text = ~formatC(n, format = "d", big.mark = ","), textposition = "outside",
  ...
  output$cat_trend <- renderPlotly({
    ...
    for (i in seq_along(cats)) {
      ...
      p <- p %>% add_trace(data = dd, x = ~Year, y = ~Count,
                           ...
                           hovertemplate = paste0("<b>", cats[i], "</b><br>%{x}: %{y:,}<extra></extra>"))
    }
    p %>% dark_plotly() %>%
      layout(xaxis = list(title = "", tickmode = "linear", dtick = 1),
             yaxis = list(title = "Crime Count"),
  ...
  output$risk_map <- renderLeaflet({
    if (is.null(bhm_rr)) {
      return(leaflet() %>% addProviderTiles(providers$CartoDB.DarkMatterNoLabels) %>%
               addControl("<div style='background:#1e293b;color:#94a3b8;padding:12px;border-radius:8px;'>
                            Run BHM model first to see risk map</div>", position = "topright") %>%
               setView(124.7, 6.4, zoom = 8))
    }
    sf_rr <- region_sf %>% left_join(bhm_rr, by = "Province")
    pal   <- colorNumeric(c("#4CAF50", "#FFF9C4", "#E53935"), domain = c(0.5, 2))
    popups_rr <- mapply(function(prov, rr, lo, hi, risk) paste0(
      "<div style='background:#1e293b;color:#e2e8f0;padding:14px;border-radius:8px;min-width:220px;'>",
      ...
      ";'>", risk, "</b></div>"
    ), sf_rr$Province, sf_rr$RR, sf_rr$RR_lower, sf_rr$RR_upper, sf_rr$Risk, SIMPLIFY = TRUE)
    leaflet(sf_rr) %>% addProviderTiles(providers$CartoDB.DarkMatterNoLabels) %>%
      addPolygons(fillColor = ~pal(RR), fillOpacity = 0.80,
                  color = "#0f172a", weight = 2, popup = popups_rr,
                  highlightOptions = highlightOptions(weight = 3, color = "#3b82f6",
                                                      fillOpacity = 0.95, bringToFront = TRUE),
  ...
  output$risk_bar <- renderPlotly({
    if (is.null(bhm_rr)) return(plotly_empty() %>% dark_plotly() %>%
                                  layout(title = list(text = "Run BHM model first", font = list(color = "#94a3b8"))))
    d <- bhm_rr %>% arrange(RR)
    plot_ly(d, x = ~RR, y = ~reorder(Province, RR), type = "scatter", mode = "markers",
            error_x = list(type = "data", symmetric = FALSE,
                           arrayminus = ~RR - RR_lower, array = ~RR_upper - RR,
                           color = "#64748b", thickness = 2),
            marker = list(color = ifelse(d$Risk == "High Risk", "#ef4444",
                                         ifelse(d$Risk == "Low Risk", "#10b981", "#f97316")),
                          size = 14, line = list(color = "white", width = 1.5)),
      ...
      add_segments(x = 1, xend = 1, y = 0, yend = nrow(d) + 0.5,
                   ...
                   name = "Average (RR=1)") %>%
      dark_plotly() %>%
      layout(xaxis = list(title = "Relative Risk (RR)"), yaxis = list(title = ""),
             showlegend = FALSE, margin = list(l = 130, t = 10, b = 40, r = 20))
  })
  
  output$risk_temporal <- renderPlotly({
    if (is.null(bst_temporal)) return(plotly_empty() %>% dark_plotly() %>%
                                        layout(title = list(text = "Run BST model first", font = list(color = "#94a3b8"))))
    plot_ly(bst_temporal) %>%
      add_ribbons(x = ~Year, ymin = ~RR_temp_lo, ymax = ~RR_temp_hi,
                  fillcolor = "rgba(59,130,246,0.15)", line = list(color = "transparent"),
                  name = "95% Credible Interval", hoverinfo = "skip") %>%
      add_trace(x = ~Year, y = ~RR_temp, type = "scatter", mode = "lines+markers",
                line = list(color = "#3b82f6", width = 2.5),
                marker = list(color = ~ifelse(RR_temp > 1, "#ef4444", "#10b981"),
                              size = 10, line = list(color = "white", width = 1.5)),
                name = "Temporal RR",
                hovertemplate = "<b>Year %{x}</b><br>Temporal RR: %{y:.4f}<extra></extra>") %>%
      add_segments(x = min(bst_temporal$Year), xend = max(bst_temporal$Year), y = 1, yend = 1,
                   line = list(color = "#64748b", dash = "dot", width = 1.5),
                   name = "Average (RR=1)") %>%
      dark_plotly() %>%
      layout(xaxis = list(title = "", tickmode = "linear", dtick = 1),
             yaxis = list(title = "Temporal RR"), hovermode = "x unified")
  })
  
  # ── Change-point ──────────────────────────────────────────
  output$cp_story_card <- renderUI({
    if (is.null(cp_results)) return(NULL)
    best_row <- if (!is.null(best_cp_month))
      cp_results %>% filter(CP_Year == best_cp_year, CP_Month == best_cp_month) %>% slice(1)
    else cp_results %>% filter(CP_Year == best_cp_year) %>% slice(1)
    rr_val   <- round(best_row$RR_Change[1], 4)
    pct_val  <- round((rr_val - 1) * 100, 1)
    dir_txt  <- if (rr_val > 1) paste0("+", pct_val, "% increase") else paste0(abs(pct_val), "% decrease")
    dir_col  <- if (rr_val > 1) "#ef4444" else "#10b981"
    sig_txt  <- best_row$Significant[1]
    display_label <- if (!is.null(best_cp_label)) best_cp_label else as.character(best_cp_year)
    tags$div(class = "story-card", tags$h4("What the Analysis Found"),
             tags$p("The Bayesian Change-Point Analysis identified ",
                    tags$b(style = "color:#3b82f6;", display_label),
                    " as the year when crime patterns significantly shifted. After this year, crime changed by ",
                    tags$b(style = paste0("color:", dir_col, ";"), dir_txt),
                    " (Relative Risk = ", tags$b(rr_val), "). This coincides with ",
                    if (best_cp_year <= 2019) "the Anti-Drug Campaign and Martial Law period."
                    else if (best_cp_year <= 2021) "the COVID-19 pandemic and related restrictions."
                    else "the post-pandemic recovery period.",
                    " The change is ",
                    tags$b(style = paste0("color:", dir_col, ";"),
                           ifelse(sig_txt == "Yes", "statistically significant",
                                  "not statistically significant")),
                    " at the 95% credible level."))
  })
  
  output$cp_box_year <- renderInfoBox(
    infoBox("When Did It Change?", ifelse(is.null(best_cp_year), "N/A", best_cp_year),
            icon = icon("bolt"), color = "green", fill = TRUE))
  output$cp_box_change <- renderInfoBox({
    val <- if (!is.null(best_cp_row) && nrow(best_cp_row) > 0) {
      rr  <- best_cp_row$RR_Change[[1]]
      pct <- round((rr - 1) * 100, 1)
      paste0(ifelse(pct > 0, "+", ""), pct, "%")
    } else "N/A"
    infoBox("Crime Change After Breakpoint", val, icon = icon("percent"), color = "orange", fill = TRUE)
  })
  output$cp_box_sig <- renderInfoBox({
    sig <- if (!is.null(best_cp_row) && nrow(best_cp_row) > 0) best_cp_row$Significant[[1]] else "N/A"
    infoBox("Is This Change Significant?", sig, icon = icon("check-circle"), color = "blue", fill = TRUE)
  })
  
  output$cp_trend <- renderPlotly({
    if (is.null(cp_results)) return(plotly_empty() %>% dark_plotly() %>%
                                      layout(title = list(text = "Run CPA model first", font = list(color = "#94a3b8"))))
    d <- crime_clean %>% count(Year, name = "Count")
    p <- plot_ly(d, x = ~Year, y = ~Count, type = "scatter",
                 mode = "lines+markers", fill = "tozeroy",
                 fillcolor = "rgba(59,130,246,0.08)",
                 line = list(color = "#3b82f6", width = 2),
                 marker = list(color = "#f97316", size = 7), name = "Observed",
                 hovertemplate = "<b>%{x}</b><br>%{y:,} crimes<extra></extra>")
    if (!is.null(best_cp_year)) {
      y_max <- max(d$Count, na.rm = TRUE)
      p <- p %>%
        add_segments(x = best_cp_year, xend = best_cp_year, y = 0, yend = y_max * 1.05,
                     line = list(color = "#10b981", width = 2.5, dash = "dash"),
                     name = paste("Change-Point:", best_cp_year)) %>%
        add_annotations(x = best_cp_year, y = y_max,
                        text = paste0("<b>", best_cp_year, " - Pattern Changed</b>"),
                        showarrow = FALSE,
                        font = list(color = "#10b981", size = 11),
                        xanchor = "left", yanchor = "top") %>%
        add_annotations(x = (min(d$Year) + best_cp_year) / 2, y = y_max * 0.5,
                        text = "Before", showarrow = FALSE,
                        font = list(color = "#64748b", size = 14), xanchor = "center") %>%
        add_annotations(x = (best_cp_year + max(d$Year)) / 2, y = y_max * 0.5,
                        text = "After", showarrow = FALSE,
                        font = list(color = "#64748b", size = 14), xanchor = "center")
    }
    p %>% dark_plotly() %>%
      layout(xaxis = list(title = "", tickmode = "linear", dtick = 1),
             yaxis = list(title = "Crime Count"), hovermode = "x unified")
  })
  
  output$cp_prov_bar <- renderPlotly({
    if (is.null(prov_cp)) return(plotly_empty() %>% dark_plotly() %>%
                                   layout(title = list(text = "Run CPA model first", font = list(color = "#94a3b8"))))
    plot_ly(prov_cp, x = ~Best_CP_Year, y = ~reorder(Province, Best_CP_Year),
            type = "bar", orientation = "h",
            marker = list(color = ifelse(prov_cp$Significant == "Yes", "#10b981", "#334155"),
                          line = list(color = "#0f172a", width = 1)),
            text = ~Best_CP_Year, textposition = "outside",
            hovertemplate = "<b>%{y}</b><br>CP Year: %{x}<extra></extra>") %>%
      dark_plotly() %>%
      layout(xaxis = list(title = "Change-Point Year", range = c(2014, 2026)),
             yaxis = list(title = ""), margin = list(l = 130, t = 10, b = 30, r = 40))
  })
  
  output$cp_pct_change <- renderPlotly({
    if (is.null(prov_cp)) return(plotly_empty() %>% dark_plotly())
    d <- prov_cp %>% mutate(Pct = round((RR_Change - 1) * 100, 1))
    plot_ly(d, x = ~Pct, y = ~reorder(Province, Pct), type = "bar", orientation = "h",
            marker = list(color = ifelse(d$Pct > 0, "#ef4444", "#10b981"),
                          line = list(color = "#0f172a", width = 1)),
            text = ~paste0(ifelse(Pct > 0, "+", ""), Pct, "%"), textposition = "outside",
            hovertemplate = "<b>%{y}</b><br>Change: %{text}<extra></extra>") %>%
      add_segments(x = 0, xend = 0, y = 0, yend = nrow(d) + 0.5,
                   line = list(color = "#64748b", dash = "dot", width = 1.5), name = "No Change") %>%
      dark_plotly() %>%
      layout(xaxis = list(title = "% Change in Crime"), yaxis = list(title = ""),
             showlegend = FALSE, margin = list(l = 130, t = 10, b = 30, r = 60))
  })
  ...
  output$data_tbl <- renderDT({
    ...
    datatable(tbl_data, filter = "top", rownames = FALSE,
              ...
              options = list(
                ...
                initComplete = JS(
                  ...
                  "}")),
              extensions = "Buttons", class = "stripe hover") %>%
      formatStyle("Count",
